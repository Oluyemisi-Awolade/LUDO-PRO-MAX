...
Single-page architecture (no routing) for reliable Android rendering.
"""
...
async def main(page: ft.Page):
    ...
    # ── ROOT CONTAINER (single-page switch) ───────────────────────────────────
    root = ft.Column(
        expand=True,
        spacing=0,
    ...
    async def play(k):
        ...
        try: await _audio[k].resume_async()
        except:
    ...
    async def init_user(uid, email):
        existing = {}
        try: existing = await fb_get(f"users/{uid}", gs["user"]["idToken"]) or {}
        except: pass
        if "email" not in existing:
            ...
            try: await fb_put(f"users/{uid}", existing, gs["user"]["idToken"])
            except: pass
    ...
    def can_move(player, t_idx, steps):
        ...
        return (steps - (len(TRACK_PATH) - pidx)) <= 6 if steps >= len(TRACK_PATH) - pidx else True

    def new_pos(player, t_idx, steps):
            ...
            for ci in range(BOARD_SIZE):
                ...
                for i in range(4):
                    if pos in nest_sets[i]:
                        bg = nest_cols[i]; matched = True; break
                if not matched:
    ...
    def refresh_game_ui():
        ...
        ud = gs["user_data"]
        if ud:
            status_txt.value = (f"🪙 {ud['coins']}  🏆 {ud['wins']}W  "
    ...
    async def show_game_over(winner_idx):
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Column([
                ...
                ft.Text(f"You: {labels[min(my_pl,3)]}",
                        size=13, color=ft.Colors.WHITE70,
                        text_align=ft.TextAlign.CENTER),
                ft.Text(f"+{rewards[min(my_pl,3)]} coins 🪙",
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # SCREENS  (each calls show() to swap content)
    # ═══════════════════════════════════════════════════════════════════════════
            ...
            ft.Container(
                ...
                content=ft.Column(
                    ...
                    controls=[
                        ...
                        mk_btn("2-Dice vs Bot 🎲🎲", lambda e: go_bot("hard", True),
                        ...
                        support_card(),
                        ft.TextButton(
                            "Logout", on_click=lambda e: show_login_screen(),
