...
async def fb_auth(email, password, signup=False):
    ...
    return await fb_request("post", url,
...
async def main(page: ft.Page):
    ...
    async def play(k):
        ...
        try:
            ...
            try: await _audio[k].play_async()
            except: pass

    async def play_bgm():
        if not _bgm_on["v"]: return
        try: await _audio["bgm"].play_async()
        except: pass

    async def stop_bgm():
        try: await _audio["bgm"].pause_async()
        except: pass
    ...
    # ── PERSISTENT WIDGETS ────────────────────
    dice1_text   = ft.Text("⚀", size=48, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE)
    dice2_text   = ft.Text("⚀", size=48, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE)
    ...
    status_bar   = ft.Text("", size=12, color=ft.Colors.WHITE70)
    board_col    = ft.Column(spacing=1, alignment=ft.MainAxisAlignment.CENTER)
    player_strip = ft.Row(wrap=True, alignment=ft.MainAxisAlignment.SPACE_AROUND, spacing=4)
    chat_list    = ft.ListView(height=88, spacing=2, auto_scroll=True)
    ...
    def compute_new_pos(player, t_idx, steps):
        ...
        si = TRACK_PATH.index(START_POS[player])
        remaining = len(TRACK_PATH) - (ti - si) % len(TRACK_PATH)
        if steps >= remaining:
            hs = steps - remaining
    ...
    async def do_move(player, t_idx, steps):
        ...
        captured = False
        np_tuple = tuple(new_pos)
        if (np_tuple not in SAFE_SPOTS and
        ...
        await play("move")
        gs["dice1"] = gs["dice2"] = 0
        dice1_text.value = dice2_text.value = "⚀"
        ...
        if all_home(player):
            ...
            if player == gs["player_index"]:
                ...
                await sync_user()
                snack(f"🏆 You finished #{place}! +{coin_reward} coins",
                      ft.Colors.GREEN_700)
    ...
    async def run_bot():
        ...
        for ti in range(4):
            ...
            if diff == "easy":
                ...
                for p, toks in gs["tokens"].items():
                    if p == bot: continue
                    if npos in toks and tuple(npos) not in SAFE_SPOTS: score += 150
                ...
                for p, toks in gs["tokens"].items():
                    if p == bot: continue
                    if npos in toks and tuple(npos) not in SAFE_SPOTS: score += 300
    ...
    def build_board():
        ...
        for pi, toks in gs["tokens"].items():
            for ti, tp in enumerate(toks):
                key = tuple(tp)
                token_map.setdefault(key, []).append((pi, ti))
            ...
            for col_idx in range(BOARD_SIZE):
                ...
                for i in range(4):
                    ...
                    if pos in SAFE_SPOTS:
                        ...
                        bg, text = "#7b1fa2", "🏠"
                    elif pos == START_POS[0]: bg = start_colors[0]
                    elif pos == START_POS[1]: bg = start_colors[1]
                    elif pos == START_POS[2]: bg = start_colors[2]
                    elif pos == START_POS[3]: bg = start_colors[3]
                ...
                if len(tokens_here) == 1:
                    ...
                    cell_content = ft.Stack(items, width=cell_size, height=cell_size)
                else:
                    cell_content = ft.Text(text, size=8, weight=ft.FontWeight.BOLD,
    ...
    def update_ui():
        ...
        for idx in range(len(gs["players"])):
            ...
            player_strip.controls.append(ft.Container(
                content=ft.Column([
                    ft.Text(PLAYER_NAMES[idx], size=10, color=ft.Colors.WHITE,
                            weight=ft.FontWeight.BOLD),
                    ft.Text(name[:10], size=9, color=ft.Colors.WHITE70),
                    ft.Text("✅" if finished else ("🎲" if active else ""), size=10)
        ...
        if gs["is_online"] and gs["room_id"]:
            try:
                await fb_patch(f"rooms/{gs['room_id']}/chat/{int(time.time()*1000)}",
                               {"player": PLAYER_NAMES[gs["player_index"]],
                                "msg": msg, "time": int(time.time())},
                               gs["user"]["idToken"])
            except: pass
        else:
            gs["chat_messages"].append(
                {"player": PLAYER_NAMES[gs["player_index"]], "msg": msg})
            update_chat()
            ...
            try:
                ...
                if room and room != last:
                    ...
                    if len(chat_data) != gs["last_chat_len"]:
                        gs["chat_messages"]  = sorted(chat_data.values(),
                                                      key=lambda x: x["time"])
    ...
    def setup_game(num_players, mode="bot", difficulty="hard", two_dice=False):
        gs.update({
            "stop_poll": True, "is_online": False, "two_dice_mode": two_dice,
    ...
    def support_card():
        ...
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Icon(ft.Icons.VOLUNTEER_ACTIVISM,
                            color=ft.Colors.ORANGE_300, size=18),
                    ft.Text("Support the Developer", size=13,
                            weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
                ], spacing=8),
                ft.Text("Ludo Pro Max is free.\nBuy the dev a coffee ☕ to keep it alive!",
            ...
            bgcolor="#2d1f0a", padding=14, border_radius=12,
            border=ft.border.all(1, ft.Colors.ORANGE_800), width=310
        )
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Column([
                ...
                ft.Text(f"{PLAYER_NAMES[winner_idx]} wins!",
                        size=17, color=PLAYER_COLORS[winner_idx],
                        text_align=ft.TextAlign.CENTER),
                ft.Divider(color="#44446a"),
                ft.Text(f"You: {labels[min(my_place, 3)]}",
                        size=13, color=ft.Colors.WHITE70,
                        text_align=ft.TextAlign.CENTER),
                ft.Text(f"+{rewards[min(my_place,3)]} coins 🪙",
                ...
                ft.Row([
                    ft.ElevatedButton(
                        ...
                        on_click=lambda e: page.run_task(do_rematch, e),
                        style=ft.ButtonStyle(
                            shape=ft.RoundedRectangleBorder(radius=10),
                            bgcolor=ft.Colors.PURPLE_700, color=ft.Colors.WHITE
                        )
                    ...
                    ft.ElevatedButton(
                        ...
                        on_click=lambda e: page.run_task(do_menu, e),
                        style=ft.ButtonStyle(
                            shape=ft.RoundedRectangleBorder(radius=10),
    ...
    def v_login():
        ef = ft.TextField(
            ...
            color=ft.Colors.WHITE, label_style=ft.TextStyle(color=ft.Colors.WHITE54)
        )
        pf = ft.TextField(
        ...
        return ft.View("/", bgcolor="#1a1a2e", padding=0, controls=[
            ft.Container(
                expand=True, bgcolor="#1a1a2e", padding=24,
                content=ft.Column(
                    ...
                    controls=[
                        ...
                        ft.Text("Ludo Pro Max", size=30, weight=ft.FontWeight.BOLD,
                                color=ft.Colors.WHITE,
                                text_align=ft.TextAlign.CENTER),
                        ft.Text(f"v{APP_VERSION}", size=11, color=ft.Colors.WHITE38,
                                text_align=ft.TextAlign.CENTER),
            ...
            ft.Container(
                ...
                content=ft.Column(
                    ...
                    controls=[
                            ...
                            content=ft.Row([
                                ft.CircleAvatar(
                                    content=ft.Text(
                                        ...
                                        color=ft.Colors.WHITE),
                                    bgcolor=ft.Colors.PURPLE_700, radius=24),
                                ft.Column([
                        ...
                        mk_btn("Easy Bot 😊",
                               lambda e: go_bot("easy"),
                               color=ft.Colors.GREEN_700),
                        mk_btn("Hard Bot 😤",
                               lambda e: go_bot("hard"),
                               color=ft.Colors.ORANGE_700),
                        mk_btn("Hardest Bot 💀",
                               lambda e: go_bot("hardest"),
    ...
    def v_game():
        ...
        dice_row = ft.Row(
            controls=[dice1_text] + ([dice2_text] if gs["two_dice_mode"] else []),
            alignment=ft.MainAxisAlignment.CENTER, spacing=10
        )
                        ...
                        ft.Container(
                            content=ft.Row([
                                ft.IconButton(
                                    ...
                                    on_click=lambda e: page.run_task(back, e)
                                ),
                                status_bar,
                                ft.Row([
                                    ft.IconButton(
    ...
    def v_tournament():
        t_list  = ft.ListView(expand=True, spacing=6, height=380)
        spinner = ft.ProgressRing(visible=True, width=26, height=26,
        ...
        page.run_task(load)
        return ft.View("/tournament", bgcolor="#1a1a2e", padding=0, controls=[
    ...
    def v_leaderboard():
        lb_list = ft.ListView(expand=True, spacing=6, height=480)
        spinner = ft.ProgressRing(visible=True, width=26, height=26,
        ...
        page.run_task(load)
        return ft.View("/leaderboard", bgcolor="#1a1a2e", padding=0, controls=[
