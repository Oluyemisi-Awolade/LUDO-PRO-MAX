...
class GameState {
  ...
  final int numPlayers;          // 2–4
  final int currentTurn;         // 0–3
  final int playerIndex;         // which seat is the human (0 for bot/local)
  final Map<int, String> playerNames;

  // tokens[playerIdx][tokenIdx] = [row, col]
  ...
  final List<int> finishedPlayers;  // ordered finish positions
  final int? winner;
...
class UserData {
  ...
  UserData copyWith({
    int? coins, int? wins, int? losses, int? games, int? elo, String? idToken,
  }) => UserData(
  ...
  factory UserData.fromJson(Map<dynamic, dynamic> j, String uid, {String? idToken}) =>
      UserData(
        uid:         uid,
        email:       j['email'] as String? ?? '',
        displayName: j['displayName'] as String? ?? j['email'] as String? ?? 'Player',
...
class ChatMessage {
  ...
  final String msg;
  final int timestamp;
  const ChatMessage({required this.player, required this.msg, required this.timestamp});
...
int botChooseToken({
  ...
  for (final ti in moves) {
    ...
    // Add small noise to hardest to avoid predictability
